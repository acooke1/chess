...
func (sq Square) File() File {
	return File(int(sq) % numberOfSquaresInRow)
}

func (sq Square) Rank() Rank {
	return Rank(int(sq) % numberOfSquaresInRow)
}
...
func (sq Square) Color() color.Color {
	if (sq/8)%2 == (sq % 2) {
		return color.Black
	} else {
		return color.RGBA{250, 250, 250, 1}
	}
}
